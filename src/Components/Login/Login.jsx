...
function Login() {
    ...
    async function login(values) {
        ...
        try {
            ...
            console.log(error)
            setIsLoading(false)
            seterror(`Try agin enter valid email and password   ${error.response.data.msg}`)
        }
    }
    ...
    let validationSchema = Yup.object({
        ...
        password: Yup.string().matches(/^[A-Z][a-z0-9]{5,15}$/, "Password Start With uppercase letter and Must be More then 3 characters").required("Enter Your Phone"),
                    ...
                    <div className="col-lg-5">
                        <div className="data shadow">
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                ...
                                {/* Btn */}
                                <button disabled={!(formik.isValid && formik.dirty)} type='submit'>  {isLoading ? <i className='fa-solid fa-spinner fa-spin'></i> : 'Login'} </button>
                                <p className='member'>Create a new account ? <Link to={'/Register'}> Register </Link></p>
                            </form>
                        </div>
                    </div>
                </div>
